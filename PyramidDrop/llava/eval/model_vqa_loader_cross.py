...
def eval_model(args):
    ...
    if model_class_name == "LlavaLlamaForCausalLM_PDrop":
        ...
        # cross-attention-sink-redistribution add-on: rebuild the pipeline objects from the
        # real CLI args (LlamaModel.__init__ only has `config` to work with - see the comment
        # there in modeling_llama_pdrop.py).
