...
from cross_attention_sink_redistribution_llava.attention_redistribution import sink_attention_redistributor
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        # sink_selector / cross_attention_importants / sink_attention_redistributor implement the
        # cross-attention-sink-redistribution add-on (see cross_attention_sink_redistribution_llava/).
        # LlamaConfig carries none of their settings, so build them from `config` here (every field
        # is getattr-with-default, so this is a harmless no-op default) - model_vqa_loader_cross.py
        # replaces them with instances built from the real CLI args right after `from_pretrained`,
        # exactly like `layer_list` / `image_token_ratio_list` below.
        self.sink_selector = sink_token_selector(self.config)
        self.cross_attention_importants = cross_attention_importants(self.config, sink_selector=self.sink_selector)
        self.sink_attention_redistributor = sink_attention_redistributor(self.config)
    ...
    def pdrop_rank_drop(
        ...
        if getattr(self.config, 'tokenizer_padding_side', 'right') == "right":
            ...
            # per-sample valid (non-pad) token count, captured before `attention_mask` below is
            # overwritten with the per-sample valid-trimmed list - used as the end of the
            # cross-attention text query span (mirrors FastV's `seq_length_with_past`, per-sample)
            ...
            for i in range(batch_size):
                ...
                # sink-token / cross-attention / attention-redistribution add-on, mirroring
                # FastV's fastv_forward layer-k pipeline (modeling_llama.py): self-attention
                # (attention_avg_text) supplies the mass being redistributed; cross-attention
                # (query = this sample's text tokens after the image, key = this sample's
                ...
                sink_budget = attention_avg_text[sink_ids].sum()
                new_image_attention = self.sink_attention_redistributor.redistribute(
                    attention_avg_text, cross_attention_importance, sink_ids, sink_budget
                )

                # rank and drop by attention score (post sink-redistribution, matching the
                # FastV reference pipeline in modeling_llama.py)
                top_rank_index = new_image_attention.topk(keep_length[i]).indices
