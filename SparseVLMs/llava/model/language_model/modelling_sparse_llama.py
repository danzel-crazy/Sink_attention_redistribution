...
from cross_attention_sink_redistribution_llava_sparsevlm.attention_redistribution import sink_attention_redistributor
...
class LlamaDynamicvitModel(LlamaModel):
    def __init__(self, config: LlamaConfig, pruning_loc=[2, 6, 15]):
        ...
        # cross-attention-sink-redistribution add-on (see cross_attention_sink_redistribution_llava_sparsevlm/).
        ...
        self.enable_cross_sink_redistribution = getattr(config, "enable_cross_sink_redistribution", False)
            ...
            if layer_idx in self.pruning_loc and len(pre_prompt_length_list) != 0 and hidden_states.shape[1] !=1:
                ...
                # Inference
                    ...
                    pred_score_vis, s_flag, relation_vis_text = attn_postprocess_topk(
                        ...
                        hidden_states=hidden_states,
                        sink_selector=self.sink_selector if self.enable_cross_sink_redistribution else None,
                        cross_attention_importants=self.cross_attention_importants if self.enable_cross_sink_redistribution else None,
                        sink_attention_redistributor=self.sink_attention_redistributor if self.enable_cross_sink_redistribution else None,
                    ) # B, L_v
