...
def attn_postprocess_topk(
    ...
    hidden_states=None,
    sink_selector=None,
    cross_attention_importants=None,
    sink_attention_redistributor=None,
):
    '''
    ...
    # cross-attention-sink-redistribution add-on: relation_vis_text (real text->visual attention,
    # already restricted to the same t_token_idx queries) is the mass being redistributed - it's
    # the thing that can be distorted by attention sinks. Cross-attention, computed from raw
    # hidden states over the same t_token_idx queries, only decides which visual tokens receive
    # the budget freed from the sink tokens.
    if sink_attention_redistributor is not None:
        visual_hidden = hidden_states[:, v_token_start:v_token_start + v_token_num, :]
        sink_ids = sink_selector._select_sink_tokens(visual_hidden[0])

        cross_attention_importants.compute_cross_attention(
            hidden_states,
            text_token_indices=t_token_idx,
            sink_local_ids=sink_ids,
            visual_tokens_start_index=v_token_start,
            visual_tokens_length=v_token_num,
        )
        importance_scores = cross_attention_importants.important_tokens_scores_raw

        # sink budget - always from the real attention, the actual mass being redistributed
        sink_budget = relation_vis_text[0][sink_ids].sum()
        redistributed = sink_attention_redistributor.redistribute(
            relation_vis_text[0], importance_scores, sink_ids, sink_budget
        )
        relation_vis_text = redistributed.unsqueeze(0)
