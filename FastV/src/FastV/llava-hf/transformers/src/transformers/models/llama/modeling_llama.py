...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        # sink_selector / cross_attention_importants / sink_attention_redistributor are built
        # lazily in fastv_forward() from the fastv_config dict passed in at call time (see
        # _ensure_fastv_pipeline below) - LlamaConfig has no sink/cross-attention/redistribution
        # fields, so there is nothing meaningful to construct them from here.
        self._fastv_pipeline_config = None
    ...
    def _ensure_fastv_pipeline(self, fastv_config):
        # fastv_config is the same dict object for the whole `generate()` call (built once in
        # inference.py and stored on LlavaConfig), so this only rebuilds when it actually changes.
        if self._fastv_pipeline_config is fastv_config:
            return
    ...
    def fastv_forward(
        ...
        self._ensure_fastv_pipeline(fastv_config)
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if use_cache:
                ...
                if layer_idx<FASTV_k:
                    ...
                    # sort index
                    keep_indexs = keep_indexs.sort().values
                    print(f"remain visual token number: {top_attention_rank_index.shape}")
                    # update seq length
                    new_seq_length = keep_indexs.shape[0]
                    # filter hidden states
